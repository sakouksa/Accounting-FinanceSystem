...
export const useFinancialReport = () => {
  ...
  const getStats = async () => {
    const res = await request('financial-reports/stats', 'get')
    if (res && !res.error) {
      setState(prev => ({
        ...prev,
        stats: res?.stats || {}
  ...
  const getList = async (custom = {}) => {
    ...
    const res = await request(`financial-reports${query}`, 'get')
  ...
  const handleDelete = record => {
    Modal.confirm({
      ...
      centered: true,
      onOk: async () => {
        const res = await request(`financial-reports/${record.id}`, 'delete')
        if (res && !res.error) {
  ...
  const handleBulkDelete = () => {
    ...
    Modal.confirm({
      ...
      centered: true,
      onOk: async () => {
        const res = await request('financial-reports/bulk-delete', 'post', {
          ids: selectedRowKeys
        })
        if (res && !res.error) {
          message.success(res.message || 'លុបជោគជ័យ!')
  ...
  const handleDeleteAll = () => {
    Modal.confirm({
      ...
      centered: true,
      onOk: async () => {
        const res = await request('financial-reports/delete-all', 'post')
        if (res && !res.error) {
